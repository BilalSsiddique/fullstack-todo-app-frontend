...
import type { Todo,todo as todoType } from "../store/slice/todoSlice";
...
const Todos = () => {
  ...
  // console.log('pagescheck',totalCount,totalPagess)
  useEffect(() => {
    // console.log("executed useEffect");
    dispatch(fetchTodos({ currentPage, itemsPerPage, token }));
  }, [currentPage, dispatch]);

  useEffect(() => {
    setCurrentPage(totalPagess);
    dispatch(fetchTodos({ totalPagess, itemsPerPage, token }));
  }, [totalPagess]);

  const deleteTod = (id: string) => {
    dispatch(deleteTodos({todoId:id,token})).then((res) => {
      // console.log("deleted", res);
      if (res.meta.requestStatus === "fulfilled") {
        // if(totalCount%5)
        dispatch(fetchTodos({ currentPage, itemsPerPage,token })).then((res) => {
          if (res.meta.requestStatus === "fulfilled") {
  ...
  const getTod = (id: string) => {
    ...
    dispatch(getTodo({id,token})).then((res) => {
      // console.log("set", res);
      if (res.meta.requestStatus === "fulfilled") {
  ...
  // console.log('totolpages',totalPagesArray)
  ...
  if (todo?.length === 0) {
    return (
      <div className="flex flex-col font-extrabold mx-auto my-auto mt-0 items-center justify-center shadow-inner w-2/4 bg-slate-200 h-[200px] text-red-400">
        ...
        <Link
          to="/create-todo"
          className="bg-slate-400 shadow-inner text-white w-fit p-1 px-2 font-semibold mt-5"
        >
          Create Todo
        </Link>
      </div>
        ...
        {Array.isArray(filteredTodos) &&
          ...
          filteredTodos.map((todoItem) => (
            <div
              ...
              key={todoItem._id}
            >
              <h2 className="font-semibold">{todoItem.title}</h2>
              <h2 className="font-light text-left line-clamp-1">
                {/* {tod.desc.length > 25
                  ? tod.desc.slice(0, 25) + "...."
                  : tod.desc} */}{" "}
                {todoItem.desc.length> 25 ? todoItem.desc.slice(0,25):todoItem.desc}
              </h2>
              <h2>{todoItem.check}</h2>
              <h2 className=" w-full flex justify-start font-semibold text-sm">
                {"Created : "} &nbsp;
                <p className="font-normal">
                  {" "}
                  {String(todoItem.date).slice(0, 10)}{" "}
                </p>
              </h2>

              <div onClick={() => getTod(todoItem._id)}>
                <TbEdit
              ...
              <div onClick={() => deleteTod(todoItem._id)}>
                <MdDeleteOutline
                ...
                <p
                  className={`top-2.5 right-3.5  absolute w-3 h-3 rounded-full ${
                    todoItem.check === true ? "bg-[#73D043]" : "bg-yellow-400"
                  }  `}
                ></p>
              </div>
            </div>
